...
class TestArtistDiscographyPlugin(PicardTestCase):
    ...
    def setUp(self):
        ...
        self.tagger.webservice.mb_api = MagicMock()
    ...
    def test_load_discography_logic(self):
        ...
        def side_effect(handler, **kwargs):
            ...
            handler(doc, None, None, tagger=self.tagger, artist_id=kwargs.get('artist'), offset=offset)

        self.tagger.webservice.mb_api.browse_releases.side_effect = side_effect

        self.plugin.load_discography(self.tagger, artist_id)

        self.assertEqual(self.tagger.webservice.mb_api.browse_releases.call_count, 2)
        self.assertEqual(self.tagger.load_album.call_count, 150)
    ...
    def test_load_discography_tool(self, mock_load, mock_getItem, mock_getText):
        ...
        self.tagger.webservice.mb_api.find_artists.side_effect = find_artists_side_effect

        self.load_tool.callback([])

        mock_getText.assert_called_once()
        self.tagger.webservice.mb_api.find_artists.assert_called_once()
